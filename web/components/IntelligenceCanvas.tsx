...
export default function IntelligenceCanvas() {
  ...
  useEffect(() => {
    ...
    function waypoints() {
      return [
        { x: 0.06, y: 0.72 }, { x: 0.14, y: 0.55 }, { x: 0.2, y: 0.68 }, { x: 0.28, y: 0.48 },
        { x: 0.36, y: 0.6 }, { x: 0.42, y: 0.4 }, { x: 0.5, y: 0.54 },
    ...
    const survCells: { r: number; c: number; alpha: number; pulse: number }[] = [];
    for (let r = 0; r < 6; r++)
      for (let c = 0; c < 4; c++)
        survCells.push({ r, c, alpha: Math.random() * 0.12 + 0.03, pulse: Math.random() * Math.PI * 2 });
    ...
    const circuits: { y: number; x: number; spd: number; w: number; a: number }[] = [];
    for (let i = 0; i < 14; i++) {
      circuits.push({
        y: 0.05 + i * 0.067,
        x: Math.random(),
    ...
    function drawGear(cx: number, cy: number, r: number, teeth: number, alpha: number, rot: number) {
      ctx!.save();
      ctx!.translate(cx, cy);
      ctx!.rotate(rot);
    ...
    function drawNavPath() {
      const wps = waypoints();
      const gx = 0.03 * W(), gy = 0.33 * H(), gw = 0.52 * W(), gh = 0.52 * H();
      ...
      wps.forEach((p) => ctx!.lineTo(p.x, p.y)); ctx!.stroke();
      ctx!.setLineDash([]);
      const totalSegs = wps.length - 1;
      const progress = (t * 0.007) % totalSegs;
      const seg = Math.floor(progress), pct = progress - seg;
      const a = wps[Math.min(seg, wps.length - 2)], b = wps[Math.min(seg + 1, wps.length - 1)];
      const rx = a.x + (b.x - a.x) * pct, ry = a.y + (b.y - a.y) * pct;
      const tg = ctx!.createRadialGradient(rx, ry, 0, rx, ry, 22);
    ...
    function drawSurveillance() {
      const sx = W() * 0.55, sy = H() * 0.03, sw = W() * 0.2, sh = H() * 0.44;
      ctx!.strokeStyle = 'rgba(167,188,227,0.14)'; ctx!.lineWidth = 1;
      ctx!.strokeRect(sx, sy, sw, sh);
      const cols = 4, rows = 6, cw = sw / cols, ch = sh / rows;
      survCells.forEach((cell) => {
        const a = cell.alpha * (0.5 + 0.5 * Math.sin(t * 0.02 + cell.pulse));
      ...
      let scanY = sy + ((t * 1.2) % (sh * 2) > sh ? sh * 2 - ((t * 1.2) % sh) : (t * 1.2) % sh);
    ...
    function drawTargeting() {
      targets.forEach((tgt, i) => {
        const x = tgt.x * W(), y = tgt.y * H(), s = tgt.size;
        const phase = Math.sin(t * 0.025 + i * 1.5);
        const alpha = tgt.locked ? 0.8 + 0.2 * phase : 0.4 + 0.2 * phase;
        const cr = tgt.locked ? '255,100,100' : '167,188,227';
        const color = 'rgba(' + cr + ',' + alpha + ')';
        const cs = s * 0.36;
        ctx!.strokeStyle = color; ctx!.lineWidth = tgt.locked ? 2 : 1.5;
        ([[-1, -1], [1, -1], [1, 1], [-1, 1]] as const).forEach((d) => {
          ctx!.beginPath();
    ...
    function drawMathOverlay() {
      equations.forEach((eq, i) => {
        const alpha = 0.1 + 0.06 * Math.sin(t * 0.018 + i * 0.7);
